...
**Version: v46 | Last Updated: May 14, 2026 (Wednesday)**

> **v46 update note (May 14, 2026 — auth root cause resolved + v275-v282 + Play Store milestone, Chat Claude):** Full auth overhaul across 8 versions (v275-v282). Root cause: Chrome Custom Tab (CCT) isolated localStorage - Gmail in-app browser cannot see main Chrome session. `getSession()` deadlocked on token refresh in CCT. Fix progression: v275 removed email readOnly; v276-v279 added diagnostics, stripped getSession() from hot path, replaced with localStorage direct-read + OTP flow; v280 added OTP code entry screen; v281 switched to OTP-only; **v282 stripped auth entirely** - name + DOB form only, no email, no OTP, no Supabase session. runCalculation() fires immediately on Reveal tap; payment gate blocks naturally. HEAD: 58c25a9, SW: qc-v282. **DB wipe completed** - deleted all non-paid auth.users; 4 records remain (all has_paid=true): quantumneurocreations@gmail.com, booyens.michelle@gmail.com, qnc.review@gmail.com, keyzer.pretorius@gmail.com. **Google Play milestone:** 2 green checkmarks confirmed May 14 (published closed testing + 12 testers opted in). 14-day clock running; apply for production ~May 28. Tester CSV (12 Gmail addresses) created. **Claude Max $200/month (20x usage) activated** - no token constraints. **Sentry:** 23 open issues, mostly reveal-diag breadcrumb diagnostic messages (intentional); JAVASCRIPT-A (RLS recursion pre-fix), JAVASCRIPT-B (OTP rate-limit), JAVASCRIPT-X (no session at checkout - expected) are real but non-blocking. May 18 Sentry free-tier deadline still pending verification. Google Play Billing implementation: pre-production, not needed for closed testing.
...
| System                   | State                                                                                                                       |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| GitHub Pages             | Live (source: `/docs` on `main`. SW **qc-v282**, narration **qc-narration-v3**)                                             |
